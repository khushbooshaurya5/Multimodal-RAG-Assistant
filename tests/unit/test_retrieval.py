...
from src.retrieval import Retriever, RetrievalFilters, is_near_duplicate
from src.schemas import Chunk, ChunkMetadata, ContentType, RetrievedChunk
from src.vectorstore import FaissVectorStore


def chunk(text: str, i: int, source: str = "doc.txt", ctype: ContentType = ContentType.TEXT) -> Chunk:
    return Chunk(
        text=text,
        metadata=ChunkMetadata(
            source=source, content_type=ctype, chunk_id=f"{source}:{i}", doc_id=source, chunk_index=i
        ),
    )
...
CORPUS = [
    ...
    chunk("Convolutional neural networks apply learned filters to images!", 1, "cnn.md"),  # near-duplicate
...
def test_threshold_filters_weak_matches(retriever: Retriever):
    ...
    assert results == []
    results = retriever.retrieve("convolutional neural networks", similarity_threshold=-1.0, top_k=10)
...
def test_metadata_filter_by_content_type(retriever: Retriever):
    results = retriever.retrieve(
        "networks speech text", top_k=5, similarity_threshold=-1.0,
...
def test_metadata_filter_by_source(retriever: Retriever):
    results = retriever.retrieve(
        "networks", top_k=5, similarity_threshold=-1.0, filters=RetrievalFilters(sources=["transformers.txt"])
    )
...
class ReverseReranker:
    ...
    def rerank(self, query: str, candidates: list[RetrievedChunk]) -> list[RetrievedChunk]:
        return [c.model_copy(update={"rerank_score": float(i)}) for i, c in enumerate(reversed(candidates))]
