...
def test_pdf_page_numbers_reach_citations(indexed_service: AssistantService):
    response = indexed_service.query(
        "self-attention over token sequences", filters=RetrievalFilters(content_types=[ContentType.PDF])
    )
...
def test_unanswerable_question_reports_no_evidence(indexed_service: AssistantService):
    response = indexed_service.query("What is the boiling point of mercury?", similarity_threshold=0.6)
...
def test_index_persists_across_service_restarts(indexed_service: AssistantService, fixtures_dir: Path):
...
def test_remove_source_updates_index(indexed_service: AssistantService):
    ...
    assert removed > 0
    response = indexed_service.query("pooling layers convolution", top_k=5, similarity_threshold=-1.0)
