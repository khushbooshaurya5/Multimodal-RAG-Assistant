...
def test_image_to_vision_to_retrieval_to_answer(indexed_service: AssistantService, fixtures_dir: Path):
    image = Image.open(fixtures_dir / "diagram.png")
    response = indexed_service.query("What does this diagram show?", image=image, image_source="diagram.png")
...
def test_image_ingestion_makes_image_searchable(indexed_service: AssistantService, fixtures_dir: Path):
    ...
    assert any(c.source == "diagram.png" and c.content_type == ContentType.IMAGE for c in response.citations)
...
def test_metadata_fallback_is_honest_end_to_end(service: AssistantService, fixtures_dir: Path):
    ...
    image = Image.open(fixtures_dir / "diagram.png")
    response = service.query("What does this diagram show?", image=image, image_source="diagram.png")
